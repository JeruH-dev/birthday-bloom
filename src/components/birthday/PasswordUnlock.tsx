/**
 * 🔐 Birthday Bloom - Password Unlock System Component
 * ---------------------------------------------------
 * Authored by: NABORAJ SARKAR (https://github.com/naborajs)
 * 
 * An emotionally engaging, glassmorphic lock screen that hides the surprise
 * until the correct passcode is input. Fits naturally directly after the splash screen.
 */
...
export const PasswordUnlock = ({ onUnlock }: PasswordUnlockProps) => {
  ...
  // Automatically expand the hint drawer after 2 failed attempts
  useEffect(() => {
    if (attempts >= 2) {
      setShowHint(true);
    }
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    if (inputClean.toLowerCase() === expectedPassword.toLowerCase()) {
      ...
      // Fire confetti burst layers
      fireConfetti({ particleCount: 150, spread: 80 });
      fireStars();

      setTimeout(() => setLocalFlash(false), 300);
      // Wait for success animations before proceeding into cinematic storytelling lines
  ...
  // Generate dynamic, context-aware hint helper if VITE_PASSWORD_HINT is empty
  const getDynamicHint = () => {
    if (config.passwordHint) return config.passwordHint;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-md">
      ...
      <motion.div
        animate={error ? { x: [0, -10, 10, -10, 10, -5, 5, 0] } : {}}
        transition={{ duration: 0.5, ease: "easeInOut" }}
        className="w-full max-w-md backdrop-blur-2xl border border-white/10 p-8 text-center shadow-[0_30px_100px_rgba(0,0,0,0.8)] relative overflow-hidden"
        style={{
      ...
      >
        <div className="flex justify-center mb-6 relative">
          <motion.div
            animate={unlocked ? { scale: [1, 1.2, 0.9, 1.1, 1], rotate: [0, 10, -10, 0] } : { scale: [1, 1.05, 1] }}
            transition={unlocked ? { duration: 0.6 } : { duration: 3, repeat: Infinity }}
            className="w-16 h-16 rounded-2xl flex items-center justify-center border text-white shadow-lg transition-colors"
            style={{
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="relative">
            <input
              ...
              placeholder={isNumericFormat ? "••••" : "Enter passcode"}
              disabled={unlocked}
          ...
          <div className="flex gap-4">
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 py-4 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 text-white/60 hover:text-white transition-all text-sm font-semibold flex-1"
            >
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-6 py-4 rounded-2xl text-black font-black uppercase text-sm tracking-widest transition-all hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100 flex-[1.5]"
              style={{
        ...
        <AnimatePresence>
          {showHint && (
            <motion.div
              ...
              className="overflow-hidden mt-6 text-left border-t border-white/5 pt-4"
            >
